...
const UNAVAILABLE_BY_DAY: Record<number, readonly string[]> = {
  ...
  3: ["12:00", "15:30", "17:30"],
  4: ["10:00", "10:30", "11:00", "14:00", "15:00", "16:00", "17:00", "18:00"],
  5: ["10:00", "11:00", "11:30", "14:00", "15:00", "16:00", "17:00", "19:00"],
...
function BookingForm({
  ...
  function handleConfirm(e: React.FormEvent) {
    ...
    const message = encodeURIComponent(
      `Merhaba, ${name.trim()} olarak ${formatDate(day.date)} saat ${time} için "${service}" randevusu almak istiyorum. Telefon: ${phone.trim()}`
    );
    window.open(`${WHATSAPP_HREF}?text=${message}`, "_blank", "noopener,noreferrer");
    setConfirmed(true);
  }
  ...
  if (confirmed) {
    return (
      <div className="animate-slide-in flex flex-col items-center justify-center py-8 text-center">
        <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500/10 text-2xl">
          ✓
        </div>
        <h3 className="text-lg font-semibold text-white">Randevu Talebi Gönderildi</h3>
        <p className="mt-2 max-w-xs text-sm text-zinc-500">
          WhatsApp üzerinden onay bekleyiniz.
        </p>
        <button
          type="button"
          onClick={onBack}
          className="mt-6 text-sm font-medium text-[#C9A962] transition-colors hover:text-[#E8D5A3]"
        >
          Yeni randevu al
        </button>
      </div>
      ...
      <div className="mb-1 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">
            {formatDate(day.date)} · <span className="font-medium text-[#C9A962]">{time}</span>
          </p>
        </div>
        <button
          type="button"
...
export default function AppointmentBooking() {
  ...
  const slots = useMemo(() => {
    ...
    const unavailable = new Set(UNAVAILABLE_BY_DAY[selectedDayIndex] ?? []);
  ...
  return (
    <>
      ...
      <Card id="booking" className="flex h-full flex-col p-4 sm:p-5 lg:p-6">
        {/* Header */}
        ...
        {/* Desktop: times/form left, calendar right | Mobile: calendar top */}
            ...
            {selectedDay && (
              <p className="animate-slide-in mt-3 text-[13px] text-zinc-500">
                ...
                <span className="font-medium text-zinc-300">
                  {selectedDay.short}, {formatDate(selectedDay.date)}
                </span>
                {!selectedDay.closed && (
            ...
            {selectedDay && !selectedTime && (
              <div className="animate-slide-in flex flex-1 flex-col">
                ...
                <p className="mt-3 text-[11px] text-zinc-600">
                  ...
                  Müsait saatler ·{" "}
                  <span className="text-zinc-700 line-through">Dolu</span>
                </p>
              </div>
            )}
